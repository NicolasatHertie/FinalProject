...
Histdep <- hist(MergedOld$Incidence, breaks= seq(0,5,by=0.1), xlab="HIV Incidence Rate", main="Figure 1: Incidence Rate")
...
IncT <- ggplot(aes(x = year, y = Incidence), data = MergedOld) + geom_point(position = "jitter") + theme_bw() + geom_smooth()
...
IncC <- ggplot2::ggplot(MergedOld, aes(Incidence, country)) + geom_point() + theme_bw()
...
L2 <- glm(DDif ~ lGDPpc + lRural + lCO2 + lHCexpend + lWater + lSanitation + lFemUnempl + lFemSchool + lLifeExpect + lDPT + lMeasles,
          data=Merged, family = 'binomial')
...
# knitr::kable(vif2, align = 'c', digits = 2, 
...
# Demo
```{r, results='asis', echo=FALSE}
# DemoWB <- droplevels(subset(Merged, !region %in% "Aggregates"))

M=gvisMotionChart(Merged, idvar='country', timevar="year", xvar="Sanitation", yvar="Incidence", sizevar="Population",  options=list(width=700, height=600), chartid="World Bank")
