...
Merged$lShFemUnempl <- log(Merged$ShFemUnempl)

# Creating a new variable with Female Employment (Inverse of Female Unemployment) by quintiles
Merged$QFemUnempl <- Merged$lShFemUnempl
Merged$QFemUnempl[Merged$lShFemUnempl<=0.0133] <-1
Merged$QFemUnempl[Merged$lShFemUnempl>0.0133 & Merged$lShFemUnempl<=0.0992] <-2
Merged$QFemUnempl[Merged$lShFemUnempl>0.0992 & Merged$lShFemUnempl<=0.204] <-3
Merged$QFemUnempl[Merged$lShFemUnempl>0.2040] <-4
...
Table 1 (see Appendix) shows the main descriptive statistics of all the variables that can be found in the cleaned dataset (number of observations, mean, standard deviation, and min and max values).
...
```{r, echo=FALSE, include=FALSE}
collinear2 <- lm(Incidence ~ GDPpc + Rural + CO2 + HCexpend + Water + Sanitation + HCexpendpc + FemUnempl + FemSchool + LifeExpect + DPT + Measles + Population, data=Merged)
vif(collinear2)
...
The dummy variable should be divided by the central Tendency - as the dependent variable is very skewed, the median is a better measure of central tendency than the mean.
...
Imputed.out <- amelia(Merged, m = 5, ts = "year", cs = "iso2c", idvars = "country", ords= c("QFemSchool", "QFemUnempl", "Dummy"))
...
Model1.out <- zelig(Dummy ~ lGDPpc + lRural + lCO2 + lHCexpend + lWater + lSanitation + lLifeExpect + lDPT + lMeasles + lFemSchool + lShFemUnempl, data = Imputed.out, model = "logit")
...
knitr::kable(labels_1, digits = 2, row.names = FALSE)
...
#### MODEL 1.2.1 - Calculating predicted probabilities for Quintiles of Female Schooling for the Scenario Uganda

Model2.out <- zelig(Dummy ~ lGDPpc + lRural + lCO2 + lHCexpend + lWater + lSanitation + lLifeExpect + lDPT + lMeasles + QFemSchool + lShFemUnempl, data = Imputed.out, model = "logit")
...
```{r, echo=FALSE, include=FALSE}
# Plotting the Predicted Probabilities
png(file="plotBotswana1.png")
plot(Botswana1.out, xlab="Quartiles", ylab="Predicted Probabilities")
dev.off()
```
...
# Plot predicted probabilities Model 1
par(mfrow=c(2,2))
...
Model3.out <- zelig(Dummy ~ lGDPpc + lRural + lCO2 + lHCexpend + lWater + lSanitation + lLifeExpect + lDPT + lMeasles + QFemUnempl + lFemSchool, data = Imputed.out, model = "logit")
...
```{r, echo=FALSE, include=FALSE}
# Plotting the Predicted Probabilities
png(file="plotMalawi2.png")
plot(Malawi2.out, xlab="Quartiles", ylab="Predicted Probabilities")
dev.off()
```
...
```{r, echo=FALSE, include=FALSE}
# Plotting the Predicted Probabilities
png(file="plotBotswana2.png")
plot(Botswana2.out, xlab="Quartiles", ylab="Predicted Probabilities")
dev.off()
```


**Predicted Probabilities for Share of Female Unemployment**
...
b.out <- zelig(lIncidence ~ lGDPpc + lRural + lCO2 + lHCexpend + lWater + lSanitation + lLifeExpect + 
                 lDPT + lMeasles + lFemSchool + lShFemUnempl, data = Imputed2.out, model = "ls")
 
summary(b.out)
 
# Estimating with robust standard errors
 
c.out <- zelig(lIncidence ~ lGDPpc + lRural + lCO2 + lHCexpend + lWater + lSanitation + lLifeExpect + lDPT + lMeasles + lFemSchool + lShFemUnempl, data = Imputed2.out, model = "ls")
 
summary(c.out)
 
# Estimating the model with fixed effects
 
d.out <- zelig(lIncidence ~ lGDPpc + lRural + lCO2 + lHCexpend + lWater + lSanitation + lLifeExpect + lDPT + lMeasles + lFemSchool + lShFemUnempl + as.factor(country), data = Imputed2.out, model = "ls")
 
summary(d.out)
 
# Estimating the model with fixed effects and robust standard errors
 
e.out <- zelig(lIncidence ~ lGDPpc + lRural + lCO2 + lHCexpend + lWater + lSanitation + lLifeExpect + lDPT + lMeasles + lFemSchool + lShFemUnempl + as.factor(country), data = Imputed2.out, model = "ls", robust=TRUE)
...
knitr::kable(labels_2, digits = 2, row.names = FALSE)
...
knitr::kable(labels_3, digits = 2, row.names = FALSE)
...
**HIV Incidence Rates per Country over Time**
``` {r, echo=FALSE, include=FALSE}
