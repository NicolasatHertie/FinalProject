...
Before the data imputation highly collinear variables had to be dropped. The level of the Variance Inflation Factors 
...
d.out <- zelig(lIncidence ~ lGDPpc + lRural + lCO2 + lHCexpend + lWater + lSanitation + lLifeExpect + lDPT + lMeasles + lFemSchool + ShFemUnempl + as.factor(country), data = Imputed2.out, model = "ls")
 
summary(d.out)
 
# Estimating the model with fixed effects and robust standard errors
 
e.out <- zelig(lIncidence ~ lGDPpc + lRural + lCO2 + lHCexpend + lWater + lSanitation + lLifeExpect + lDPT + lMeasles + lFemSchool + ShFemUnempl + as.factor(country), data = Imputed2.out, model = "ls", robust=TRUE)
 
summary(e.out)
 
#install.packages("mitools")
#library("mitools")
# imp.data <- imputationList(Imputed2.out$imputations)
# mitools.fit <- MIcombine(with(imp.data, lm(Incidence ~ lGDPpc + lRural + lCO2 + lHCexpend + lWater + lSanitation + lLifeExpect + lDPT + lMeasles + Inverse + FemSchool + year))
# summary(mitools.fit)                         
...
# Showing Linear pooled OLS regression in output table with nice labels
Variables <- c("Constant", "GDP per capita", "Share of Rural Population", "Share of CO2", "Healthcare Expenditure", "Access to Water", "Access to Sanitation", "Life Expectancy", "Immunisation against DPT", "Immunisation against Measles", "Female School Enrollment", "Share of Female Unemployment")
labels_2 <- as.data.frame(summary(b.out)$coef)
labels_2$Variables <- Variables
labels_2 <- MoveFront(labels_2, "Variables")
names(labels_2) <- c("Variables", "Coefficients", "Std. Error", "T-Statistic", "P-Value")
knitr::kable(labels_2, digits = 2, row.names = FALSE, caption = "OLS Regression Results of Model 2")
