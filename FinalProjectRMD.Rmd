...
pkgs <- c("RJSONIO","WDI","dplyr","tidyr","knitr","DataCombine","lmtest","httr","XML","plyr","Amelia","XLConnect","countrycode","ggplot2","magrittr","fmsb","car","stargazer","Zelig","RCurl","reshape2","gridExtra")
...
According  to Antonovsky's model, the health of an individual depends on its *general resistance resources*. These resources are developed during the life course of an individual and they are influenced by the environment and the socio-cultural context of the individual. These resistance resources are responsible for an individual's coping ability with potential stressors (risk factors) on the psychosocial, physical and biochemical level [@Hurrelmann1989].
...
Eshetu and Woldesenbet outline their main criticism of Dahlgren's model as being that it does not take into account the special circumstances of developing countries [@Eshetu2011]. The authors question the idea of the universality of health determinants and claim that the context of health determinants in the world???s poorest countriesis very different to the context of high-income countries. One of their main arguments is that the strategies for reducing health inequalities are different in developing and developed countries. To illustrate this points, Eshetu and Woldesenbet explain that the some causes of inequality such as extreme poverty, tribalism, lack of good governance, corruption, illiteracy and colonial heritage are only relevant to the African context [@Eshetu2011].

However, this statement highly depends on the operationalization of the framework. This shortcoming can be overcome by translating the theoretical model into specific indicators that reflect the differences between developed and developing countries. Furthermore, this paper will use Dahlgren's framework only as a guideline to cluster and categorize the factors influencing HIV/AIDS incidences and prevalence rates.

The levels of Dahlgren's "Rainbow Model" should not be perceived as autonomous and independent from each other. In fact, there is a sginificant amount of literature exploring the linkages between the different levels [@Solar2010].
...
# Creating a variable with the share of Females Unemployed of the Total Number of Unemployed
...
Looking at the incidence rate at the country level (Figures 11 and 12 in Appendix) it can be seen which countries are not following the general trend. While most countries have an HIV/AIDS incidence rate slightly above 0, some outliers (countries with high incidence rates) can be identified (Figure 11 in Appendix).

A similar pattern can be detected when analysing the direction of the change of the HIV/AIDS incidence rate compared to the previous year by country (Figure 12 in Appendix). To analyse the change of the incidence rate from one year to the following, the incidence variable was lagged by one year and a new variable, calculating the difference between the lagged and the original incidence rate was created. 
...
**Figure 3: Distribution of HIV/AIDS Incidence Rate**
...
**Figure 4: Dummy Variable for High and Low HIV/AIDS Incidence**
...
**Figure 5: Logged HIV/AIDS Incidence Rate for Countries lying above the Median**
...
In the following, four case studies are presented to develop a better understanding of "best" practices and "bad" practices among countries with high changes in HIV/AIDS incidence rates between 2000 and 2012. The cases were selected by the "extreme case selection method" on the basis of peculiar positive or negative changes in incidence rates over time (see Figure 12 in Appendix) [@Gerring2008].

Figure 6 shows the four case studies that were selected: two "best" practices (green colour), namely Botswana and Malawi and two "bad" practices (red colour), Uganda and Lesotho. 

**Figure 6: Interesting Cases for Change in HIV Incidence Rates**
...
# Looking at the evolution of incidence rates in the selected countries
...
# Create a subset with the data of the case studies
CaseStudies <- Merged[which (Merged$country=="Botswana" | Merged$country=="Lesotho" | Merged$country=="Malawi" | Merged$country=="Uganda"), ]

# Plotting the HIV incidence rates of our case studies 
Cases2 <- ggplot(data=CaseStudies, aes(x=year, y=Incidence, group=country, color=country, shape=country)) + geom_line() + geom_point() + ylab( "Incidence") + xlab("Year")
...
# Creating a code to differentiate between interesting cases (0), good practices (1) and bad practices (2).  
HighIncidence$CaseStudy <- HighIncidence$Incidence
HighIncidence$CaseStudy[HighIncidence$country=="Zimbabwe" | HighIncidence$country=="Zambia"| HighIncidence$country=="Swaziland" | HighIncidence$country=="South Africa" | HighIncidence$country=="Namibia" | HighIncidence$country=="Mozambique"] <-0
...
# Plotting incidence rates of interesting cases color coded based on the index created above
Cases3 <- ggplot(data=HighIncidence, aes(x=year, y=Incidence, group=country, color=CaseStudy)) + geom_line() + geom_point() + ylab( "Incidence") + xlab("Year") + scale_color_gradient2(midpoint=1, low="grey", mid="darkgreen", high="darkred") + theme(legend.position="none")  
ggsave("figb.png", width=7, height=4, dpi=100)                                                                                                                    
Cases3 <- Cases3 + theme(plot.title = element_text(lineheight=3, face="bold"))
print(Cases3)
...
# Descriptive statistics plots focusing on the case studies
...
```{r, echo=FALSE, include=FALSE}
# Testing for multicollinearity (2nd time)
# Run the regression
...
**Figure 10: Predicted Probabilities for Female School Enrollment**
...
**Figure 11: HIV Incidence Rates per Country over Time**

![Figure3](fig3.png)

\pagebreak

**Figure 12: Change in HIV Incidence Rate compared to Previous Years per Country**
![Figure4](fig4.png)


\pagebreak

**Figure 13: Scatterplot of variables for socio-economic, cultural and environmental conditions**
...
**Figure 14: Scatterplot of variables for living and working conditions**
...
**Figure 15: Scatterplot of variables for individual lifestyle factors**
...
**Figure 16: Histogram of Female Unemployment compared to Total Unemployment (not logged)**

```{r, echo=FALSE}
hist(Merged$ShFemUnempl,xlab="Female Unemployment compared to Total Unemployment", main=NULL)                
```
...
**Figure 17: Data Imputation Matrix**
...
**Figure 18: Testing for Heteroscedasticity - Model 1**
...
**Figure 19: Testing for Heteroscedasticity - Model 2**
