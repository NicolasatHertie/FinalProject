...
ggsave("fig6.png", width=7, height=5, dpi=100)
```
...
# Creating a variable with the lFemUnempl from FemUnempl
Merged$lFemUnempl <- Merged$FemUnempl
Merged$lFemUnempl <- log(Merged$lFemUnempl)


# Creating a new variable with Female Employment (lFemUnempl of Female Unemployment) by quintiles
Merged$QFemUnempl <- Merged$lFemUnempl
Merged$QFemUnempl[Merged$lFemUnempl>2.53] <-1
Merged$QFemUnempl[Merged$lFemUnempl<2.53 & Merged$lFemUnempl>=2.01] <-2
Merged$QFemUnempl[Merged$lFemUnempl<2.01 & Merged$lFemUnempl>=1.46] <-3
Merged$QFemUnempl[Merged$lFemUnempl<1.46] <-4
...
Imputed.out <- amelia(Merged, m = 5, ts = "year", cs = "iso2c", idvars = "country", ords= c("QFemSchool", "Dummy", "QFemUnempl","DDif"))
...
Model1.out <- zelig(Dummy ~ lGDPpc + lRural + lCO2 + lHCexpend + lWater + lSanitation + lLifeExpect + lDPT + lMeasles + lFemUnempl + lFemSchool, data = Imputed.out, model = "logit")
...
Model2.out <- zelig(Dummy ~ lGDPpc + lRural + lCO2 + lHCexpend + lWater + lSanitation + lLifeExpect + lDPT + lMeasles + lFemUnempl + QFemSchool, data = Imputed.out, model = "logit")
...
Model3.out <- zelig(Dummy ~ lGDPpc + lRural + lCO2 + lHCexpend + lWater + lSanitation + lLifeExpect + lDPT + lMeasles + QFemUnempl + FemSchool, data = Imputed.out, model = "logit")
...
b.out <- zelig(lIncidence ~ lGDPpc + lRural + lCO2 + lHCexpend + lWater + lSanitation + lLifeExpect + lDPT + lMeasles + lFemUnempl + lFemSchool, data = Imputed2.out, model = "ls")
 
summary(b.out)
 
# Estimating with robust standard errors
 
c.out <- zelig(lIncidence ~ lGDPpc + lRural + lCO2 + lHCexpend + lWater + lSanitation + lLifeExpect + lDPT + lMeasles + lFemUnempl + lFemSchool, data = Imputed2.out, model = "ls")
 
summary(c.out)
 
# Estimating the model with fixed effects
 
d.out <- zelig(lIncidence ~ lGDPpc + lRural + lCO2 + lHCexpend + lWater + lSanitation + lLifeExpect + lDPT + lMeasles + lFemUnempl + lFemSchool + as.factor(country), data = Imputed2.out, model = "ls")
 
summary(d.out)
 
# Estimating the model with fixed effects and robust standard errors
 
e.out <- zelig(lIncidence ~ lGDPpc + lRural + lCO2 + lHCexpend + lWater + lSanitation + lLifeExpect + lDPT + lMeasles + lFemUnempl + lFemSchool + as.factor(country), data = Imputed2.out, model = "ls", robust=TRUE)
...
# mitools.fit <- MIcombine(with(imp.data, lm(Incidence ~ lGDPpc + lRural + lCO2 + lHCexpend + lWater + lSanitation + lLifeExpect + lDPT + lMeasles + lFemUnempl + FemSchool + year))
# summary(mitools.fit)                         
