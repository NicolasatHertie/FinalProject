...
#IncT <- ggplot(aes(x = year, y = Incidence, reorder(Incidence, value), value)), data = Merged) + geom_point(position = "jitter") + theme_bw() + geom_smooth()
#IncT <- IncT + ggtitle("Figure 2: Incidence Rate over Time") + theme(plot.title = element_text(lineheight=3, face="bold"))
#IncT <- transform(IncT, variable=reorder(Incidence, -value) ) 
...
L1 <- glm(DDif ~ lGDPpc + lRural + lCO2 + lHCexpend + lWater + lSanitation + lUnemploym + lHCexpendpc + Inverse + lFemSchool + lLifeExpect + lDPT + lMeasles,
...
knitr::kable(vif1, align = 'c', digits = 2,
...
knitr::kable(vif3, align = 'c', digits = 2,
      caption = 'Variance Inflation Factors 3')
...
```{r, echo=FALSE, type = 'latex', header = FALSE, error=FALSE}
# Looking at confidence intervals
confidence <- confint(L3)

knitr::kable(confidence, align = 'c', digits = 2,
      caption = 'Confidence Intervals 1')
```
...
stargazer(L4, L5, L6, title="Logistic Regression Results", type="latex",  font.size = "scriptsize")
...
```{r, echo=FALSE, results="asis", error=FALSE}
#### Trying to get significant results ;)

L9 <- lm(Incidence ~ lGDPpc + lRural + lCO2 + lHCexpend + lWater + lSanitation + lLifeExpect + lDPT + lMeasles + Inverse + lFemSchool, data=Merged)
...
L10 <- glm(Dummy ~ lGDPpc + lRural + lCO2 + lHCexpend + lWater + lSanitation + lLifeExpect + lDPT + lMeasles + Inverse + as.factor(QFemSchool), data=Merged, family='binomial')


# Regressing the model on FemSchool fixing the other indendent variables at Uganda's mean
Predicted_10 <- with(Merged,
                  data.frame(lGDPpc = 7.003,
                             lRural = 4.461,
                             lCO2 = -2.52,
                             lHCexpend = 2.125,
...
# Calculating the predicted probabilities
Predicted_10 <- predict(L10, newdata = Predicted_10,
                            type = 'response')
...
ggplot(data=HighIncidence, aes(x=year, y=Incidence, group=country, color=country)) + geom_line() + geom_point()
```
...
ggplot(data=CaseStudies, aes(x=year, y=Water, group=country, color=country)) + geom_line() + geom_point()

ggplot(data=CaseStudies, aes(x=year, y=Sanitation, group=country, color=country)) + geom_line() + geom_point()

ggplot(data=CaseStudies, aes(x=year, y=GDPpc, group=country, color=country)) + geom_line() + geom_point()

ggplot(data=CaseStudies, aes(x=year, y=FemSchool, group=country, color=country, shape=country)) + geom_line() + geom_point()

ggplot(data=CaseStudies, aes(x=year, y=FemUnempl, group=country, color=country, shape=country)) + geom_line() + geom_point()
...
summary(Merged$Incidence)
AboveMean <- Merged[which (Merged$Incidence>=0.3493), ] 


hist(AboveMean$Incidence)
AboveMean$lIncidence <- log(AboveMean$Incidence)
hist(AboveMean$lIncidence)

shapiro.test(AboveMean$lIncidence)
summary(AboveMean$Incidence)

N1 <- lm(Incidence ~ lGDPpc + lRural + lCO2 + lHCexpend + lWater + lSanitation + lLifeExpect + lDPT + lMeasles + Inverse + lFemSchool, data=AboveMean)
summary(N1)
