...
ggsave("fig7.png", width=7, height=5, dpi=100)
...
GDPpc <- ggplot(data=CaseStudies, aes(x=year, y=GDPpc, group=country, color=country)) + geom_line() + geom_point()
...
ggsave("fig10.png", width=7, height=5, dpi=100)

CaseStudies$FemUnempl <- as.numeric(CaseStudies$FemUnempl)

FemUnempl <- ggplot(data=CaseStudies, aes(x=year, y=FemUnempl, group=country, color=country, shape=country)) + geom_line() + geom_point()
...
ggsave("fig11.png", width=7, height=5, dpi=100)
...
ggsave("fig12.png", width=7, height=5, dpi=100)

Rural <- ggplot(data=CaseStudies, aes(x=year, y=Rural, group=country, color=country, shape=country)) + geom_line() + geom_point()
...
ggsave("fig13.png", width=7, height=5, dpi=100)

Incidence <- ggplot(data=CaseStudies, aes(x=year, y=Incidence, group=country, color=country, shape=country)) + geom_line() + geom_point()
...
ggsave("fig14.png", width=7, height=5, dpi=100)
...
```{r, echo=FALSE}
#svg("Both.svg")
#predicted.combine <- grid.arrange(Incidence, HCE, ncol = 2, main = "Incidence and HCE")
#dev.off()
...
# Drop Female Unemployment because of collinearity with inverse
...
# Creating dependent variable as Dummy (0 if above the mean, 1 if below the mean of the Incidence variable)
...
plot(Botswana2.out)
dev.off()
BotswanaB <- plot(Botswana2.out)
```


```{r, echo=FALSE, include=FALSE}

#### TRYING TO PUT GRAPHS TOGETHER
# Grid arrange
#install.packages("grid")
#install.packages("gridExtra") 
#library(gridExtra)
#predicted.combine <- grid.arrange(Malawi1.out, Lesotho1.out, Uganda1.out,Botswana1.out, ncol = 4, left = "Predicted Probability of High HIV Incidence")
#predicted.combine <- grid.arrange(Lesotho1.out, Uganda1.out, ncol = 2, left = "Predicted Probability of High HIV Incidence")
...
#predicted.combine <- grid.arrange(BotswanaB, MalawiB, ncol = 2, main = "Incidence and HCE")
#pdf(file="fig15.pdf")
#print(predicted.combine)
```
...
b.out <- zelig(Incidence ~ lGDPpc + lRural + lCO2 + lHCexpend + lWater + lSanitation + lLifeExpect + lDPT + lMeasles + Inverse + FemSchool, data = Imputed2.out, model = "ls")
 
summary(b.out)
 
# Estimating with robust standard errors
 
c.out <- zelig(Incidence ~ lGDPpc + lRural + lCO2 + lHCexpend + lWater + lSanitation + lLifeExpect + lDPT + lMeasles + Inverse + FemSchool, data = Imputed2.out, model = "ls")
 
summary(c.out)
 
# Estimating the model with fixed effects
 
d.out <- zelig(Incidence ~ lGDPpc + lRural + lCO2 + lHCexpend + lWater + lSanitation + lLifeExpect + lDPT + lMeasles + Inverse + FemSchool + as.factor(country), data = Imputed2.out, model = "ls")
 
summary(d.out)
 
# Estimating the model with fixed effects and robust standard errors
 
e.out <- zelig(Incidence ~ lGDPpc + lRural + lCO2 + lHCexpend + lWater + lSanitation + lLifeExpect + lDPT + lMeasles + Inverse + FemSchool + as.factor(country), data = Imputed2.out, model = "ls", robust=TRUE)
...
knitr::kable(as.data.frame(summary(c.out)$coef))
